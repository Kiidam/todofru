...
// POST /api/cuentas-por-pagar/[id]/pagos - Registrar pago
  ...
  try {
    ...
    if (!cuenta) {
      return NextResponse.json(
        { success: false, error: 'Cuenta por pagar no encontrada' },
        { status: 404 }
      );
    }
    ...
    const cuentaCompleta = await prisma.cuentaPorPagar.findUnique({
      where: { id },
      include: {
