    ...
    <div className="p-6 space-y-6">
      ...
      <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-200">
        <form onSubmit={submitMerma} className="grid grid-cols-1 md:grid-cols-5 gap-3">
          <select value={form.productoId} onChange={e=>setForm(prev=>({ ...prev, productoId: e.target.value }))} className="px-3 py-2 border border-gray-300 rounded-md text-black bg-white" required>
            <option value="">Producto</option>
            {catalogo?.productos?.map(p => (<option key={p.id} value={p.id}>{p.nombre}</option>))}
          </select>
          <select value={form.tipoMermaId} onChange={e=>setForm(prev=>({ ...prev, tipoMermaId: e.target.value }))} className="px-3 py-2 border border-gray-300 rounded-md text-black bg-white" required>
            <option value="">Tipo de Merma</option>
            {catalogo?.tipos?.map(t => (<option key={t.id} value={t.id}>{t.nombre}</option>))}
          </select>
          <select value={form.causaMermaId} onChange={e=>setForm(prev=>({ ...prev, causaMermaId: e.target.value }))} className="px-3 py-2 border border-gray-300 rounded-md text-black bg-white">
          ...
          <input type="number" step="0.01" value={form.cantidad} onChange={e=>setForm(prev=>({ ...prev, cantidad: parseFloat(e.target.value) || 0 }))} placeholder="Cantidad" className="px-3 py-2 border border-gray-300 rounded-md text-black bg-white" />
          <select value={form.clasificacion} onChange={e=>setForm(prev=>({ ...prev, clasificacion: e.target.value }))} className="px-3 py-2 border border-gray-300 rounded-md text-black bg-white">
            <option value="NORMAL">Normal</option>
            <option value="EXTRAORDINARIA">Extraordinaria</option>
          </select>
          <div className="md:col-span-5 flex gap-2">
            <input value={form.observaciones} onChange={e=>setForm(prev=>({ ...prev, observaciones: e.target.value }))} placeholder="Observaciones" className="flex-1 px-3 py-2 border border-gray-300 rounded-md text-black bg-white" />
            <button type="submit" disabled={submitting} className="px-4 py-2 bg-green-600 text-white rounded-md flex items-center gap-2 hover:bg-green-700">
      ...
      <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-200">
        <div className="flex flex-col md:flex-row md:items-end gap-3">
          <div>
            ...
            <input type="date" value={from} onChange={e=>setFrom(e.target.value)} className="px-3 py-2 border border-gray-300 rounded-md text-black bg-white" />
          </div>
          <div>
            <label className="text-sm font-medium text-gray-700">Hasta</label>
            <input type="date" value={to} onChange={e=>setTo(e.target.value)} className="px-3 py-2 border border-gray-300 rounded-md text-black bg-white" />
          </div>
          <div className="flex gap-2">
            <button onClick={fetchRows} className="px-4 py-2 bg-gray-100 text-black rounded-md border border-gray-300 hover:bg-gray-200">Buscar</button>
            <a href={`/api/reportes/mermas?from=${from || ''}&to=${to || ''}&format=csv`} className="px-4 py-2 bg-green-600 text-white rounded-md hover:bg-green-700">Exportar CSV</a>
          </div>
        </div>
      </div>
