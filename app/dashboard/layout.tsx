...
export default function DashboardLayout({ children }: DashboardLayoutProps) {
  ...
  const navigation = [
    ...
    {
      name: 'Agrupador de Productos',
      href: '/dashboard/agrupador-productos',
      icon: (
        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div className={`fixed inset-y-0 left-0 z-50 w-64 bg-white shadow-lg transform ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} transition-transform duration-300 ease-in-out lg:translate-x-0 lg:static lg:inset-0 lg:flex-shrink-0`}>
        <div className="flex items-center justify-center h-16 px-4 bg-green-600">
          <div className="flex items-center">
            <div className="w-8 h-8 rounded-full bg-orange-500 flex items-center justify-center relative mr-3">
              <div className="w-3 h-3 absolute -top-1 -right-1 bg-green-600 rounded-full transform rotate-45"></div>
            </div>
            <h1 className="text-xl font-bold text-white">TodoFrut</h1>
          </div>
        </div>
